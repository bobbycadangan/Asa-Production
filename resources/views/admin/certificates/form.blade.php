...
@section('title', $item->exists ? 'Edit Sertifikat' : 'Tambah Sertifikat')

@section('content')
<form method="POST" action="{{ $item->exists ? route('admin.certificates.update', $item) : route('admin.certificates.store') }}" enctype="multipart/form-data">
  @csrf
  ...
  <div class="card shadow-sm">
    <div class="card-body row g-3">
      <div class="col-md-6">
        <label class="form-label">Nama Sertifikat (opsional)</label>
        <input type="text" name="name" class="form-control" value="{{ old('name', $item->name) }}" placeholder="mis. ISO 9001">
      </div>
      <div class="col-md-6">
        <label class="form-label">Logo / Badge {{ $item->exists ? '' : '*' }}</label>
