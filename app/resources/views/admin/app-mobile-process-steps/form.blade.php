...
@section('content')
<form method="POST" action="{{ $item->exists ? route('admin.app-mobile-process-steps.update', $item) : route('admin.app-mobile-process-steps.store') }}">
  @csrf
  ...
  <div class="mt-3">
    ...
    <a href="{{ route('admin.app-mobile-process-steps.index') }}" class="btn btn-outline-secondary">Batal</a>
  </div>
</form>
@endsection
